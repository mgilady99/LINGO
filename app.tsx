...
import { Mic, MicOff, Headphones, AlertCircle, Globe, Settings } from 'lucide-react';
// 🔴 נתיבים באותיות קטנות בלבד
...
const App: React.FC = () => {
  ...
  const startConversation = async () => {
    ...
    if (!apiKey) { setError('Missing API Key in Cloudflare.'); setStatus(ConnectionStatus.ERROR); return; }
  ...
  return (
    <div className="h-screen w-screen bg-slate-950 flex flex-col md:flex-row text-slate-200 overflow-hidden">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 relative">
        ...
        <div className="mt-8 text-center max-w-md px-4">
          <h2 className="text-3xl md:text-5xl font-black text-white tracking-tighter leading-tight">{isSpeaking ? 'Gemini Speaking' : status === ConnectionStatus.CONNECTED ? 'Listening...' : selectedScenario.title}</h2>
