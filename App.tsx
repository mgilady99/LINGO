...
import { Mic, MicOff, Headphones, LogOut, AlertCircle, Globe, PanelLeftOpen } from 'lucide-react';
...
// ✅ רשימת שפות מעודכנת לפי הסדר שביקשת
  ...
  const startConversation = useCallback(async () => {
    ...
    try {
      ...
      ws.onmessage = async (ev) => {
        if (ev.data instanceof Blob) return; // הגנה משגיאת [object Blob]
        try {
        ...
        <div className="w-full max-w-xl mb-12 z-10 text-center">
            {status === ConnectionStatus.CONNECTED ? (
                <div className="flex gap-4 justify-center">
                    <button onClick={() => setIsMuted(!isMuted)} className={`px-8 py-4 rounded-3xl border-2 font-black flex items-center gap-3 ${isMuted ? 'bg-red-500/20 border-red-400' : 'bg-slate-800 border-slate-700'}`}>
                ...
                <button onClick={startConversation} className="w-full bg-indigo-600 py-6 rounded-3xl font-black text-xl shadow-2xl hover:bg-indigo-500 active:scale-95 transition">START LIVE SESSION</button>
            )}
        </div>

        <Avatar state={status !== ConnectionStatus.CONNECTED ? 'idle' : isSpeaking ? 'speaking' : isMuted ? 'thinking' : 'listening'} />

        <div className="mt-8 text-center"><h2 className="text-3xl md:text-5xl font-black">{isSpeaking ? 'Gemini Speaking' : 'LingoLive'}</h2></div>
      </main>
    </div>
  );
};
