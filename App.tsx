...
// ✅ זהו השם המדויק הנתמך ב-Live API (ללא -002)
    ...
    <div className="h-dvh w-dvw bg-slate-950 text-slate-200 flex flex-col md:flex-row overflow-hidden font-sans">
      <aside className="w-80 bg-slate-900 p-6 border-r border-white/5 flex flex-col gap-6 hidden md:flex shadow-2xl">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-black italic tracking-tighter text-indigo-400">LingoLive</h1>
        </div>
        
        <div className="space-y-6">
          <div className="space-y-1">
             <label className="text-[10px] text-slate-500 font-bold uppercase tracking-widest flex items-center gap-2"><Globe size={12}/> Native Language</label>
          ...
          <div className="space-y-1">
             <label className="text-[10px] text-slate-500 font-bold uppercase tracking-widest flex items-center gap-2"><Globe size={12}/> Target Language</label>
          ...
          <div className="pt-4 space-y-2">
            ...
            {SCENARIOS.map(s => (
              <button key={s.id} onClick={() => setSelectedScenario(s)} className={`w-full p-4 rounded-2xl border text-left text-xs transition-all ${selectedScenario.id === s.id ? 'bg-indigo-600/20 border-indigo-500 shadow-inner text-white' : 'bg-slate-800/40 border-transparent text-slate-400 hover:bg-slate-800'}`}>
                {s.icon} {s.title}
              </button>
            ))}
          </div>
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 relative">
        ...
        <div className="mt-10 flex gap-4">
          {status === ConnectionStatus.CONNECTED ? (
            <button onClick={stopConversation} className="px-12 py-4 bg-red-600 rounded-3xl font-black text-lg shadow-xl transition-all active:scale-95">STOP SESSION</button>
          ) : (
